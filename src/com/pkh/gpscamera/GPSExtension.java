...
public class GPSExtension extends AndroidNonvisibleComponent {
    ...
    private void drawByTemplate(android.graphics.Canvas canvas, String addr, String lat, String lon,
        ...
        try {
            ...
            os1.close(); os2.close(); is1.close(); is2.close();
        ...
        cardPath.quadTo(left + cardWidth, top + cardHeight, left + cardWidth - r, top + cardHeight);
        cardPath.lineTo(left + r, top + cardHeight);
        cardPath.quadTo(left, top + cardHeight, left, top + cardHeight - r);
        cardPath.lineTo(left, top + r);
        cardPath.quadTo(left, top, left + r, top);
        cardPath.close();
        ...
        float hHeight = 20 * dp;
        float hWidth = textW + (hPadding * 2) + (20 * dp);
        ...
        headerPath.quadTo(hLeft + hWidth, hTop, hLeft + hWidth - radius, hTop);
        headerPath.lineTo(hLeft + radius, hTop);
        headerPath.quadTo(hLeft, hTop, hLeft, hTop + radius);
        ...
        try {
            ...
            if (icon != null) {
                canvas.drawBitmap(icon, null, new RectF(headerIconX, headerIconY, headerIconX + iconSize, headerIconY + iconSize), null);
                icon.recycle();
            }
        } catch (Exception e) { e.printStackTrace(); }
            ...
            if (map != null) {
                ...
                try {
                    ...
                    if (pin != null) {
                        ...
                        canvas.drawBitmap(pin, null, new RectF(cx - pinSize / 2, cy - pinSize - (10 * dp), cx + pinSize / 2, cy - (10 * dp)), null);
                        pin.recycle();
                    }
                } catch (Exception e) { e.printStackTrace(); }
            }
        ...
        StaticLayout layout;
        String wilayah = getWilayahIndonesia(addr);
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            layout = StaticLayout.Builder.obtain(wilayah, 0, wilayah.length(), title, (int)(cardWidth - mapSize - 40*dp)).build();
        } else {
            layout = new StaticLayout(wilayah, title, (int)(cardWidth - mapSize - 40*dp), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
        }
        ...
        float y = titleY + layout.getHeight() + (2 * dp);
        canvas.save();
    ...
    private String fetchAddress(String lat, String lon) {
        try {
            ...
            String desa = address.optString("hamlet", address.optString("village", address.optString("suburb", address.optString("neighbourhood", ""))));
            String kecamatan = address.optString("subdistrict", address.optString("town", ""));
            String kabupaten = address.optString("county", address.optString("city", ""));
            String provinsi = "Jawa Timur";
            if (kecamatan.equals("") || kecamatan.equals(kabupaten)) { kecamatan = kabupaten; }
            return desa + "," + kecamatan + "," + kabupaten + "," + provinsi;
        } catch (Exception e) { return "Lokasi tidak ditemukan"; }
    }
    ...
    private String getWilayahIndonesia(String addr) {
        try {
            ...
            if (kabupaten.equals("")) { kabupaten = kecamatan; }
            return desa + ", Kab. " + kabupaten + ", " + provinsi + ", Indonesia 🇮🇩";
        } catch (Exception e) { return "Indonesia 🇮🇩"; }
    }
