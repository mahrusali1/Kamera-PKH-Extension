...
@DesignerComponent(version = 1,
    description = "Ekstensi GPS Map Camera PKH - Watermark Otomatis untuk Pendamping Sosial",
...
public class GPSExtension extends AndroidNonvisibleComponent {
    ...
    @SimpleFunction(description = "Proses foto dengan menambahkan watermark Alamat, GPS, dan Waktu.")
    public void ProsesFotoPKH(final String pathGambar, final String latInput, final String lonInput, final String folderSimpan, final String namaFile) {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    // 1. Ambil Koordinat (Gunakan input manual atau baca dari EXIF foto)
                    ...
                    // 2. Ambil Nama Alamat/Desa via OpenStreetMap (Reverse Geocoding)
                    ...
                    pTeks.setTextSize(bitmapAsli.getWidth() / 28); // Ukuran proporsional
                    pTeks.setShadowLayer(5f, 0f, 0f, Color.BLACK);
                    pTeks.setAntiAlias(true);

                    // Background Box Hitam Transparan di bawah
                    ...
                    // 4. Simpan Hasil ke Folder Tujuan
                    File direktori = new File(folderSimpan);
                    if (!direktori.exists()) direktori.mkdirs();
                    File fileHasil = new File(direktori, namaFile);
                    ...
                    // Kirim hasil kembali ke App Inventor
    ...
    private String ambilAlamat(String lat, String lon) {
        ...
        try {
            ...
            conn.setRequestProperty("User-Agent", "AplikasiKameraPKH");
        ...
        } catch (Exception e) { return "Alamat gagal dimuat (Cek Internet)"; }
    }

    @SimpleEvent(description = "Dipanggil saat proses foto berhasil.")
    public void Selesai(String path) { EventDispatcher.dispatchEvent(this, "Selesai", path); }

    @SimpleEvent(description = "Dipanggil saat terjadi error.")
    public void Error(String pesan) { EventDispatcher.dispatchEvent(this, "Error", pesan); }
}
